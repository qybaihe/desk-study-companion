...
while True:
    ...
    if (
        ...
        child_line = "CHILD: " + (
            "PRESENT" if presence["present"] else "AWAY"
        )
        oled.text(
            child_line, max(0, (128 - len(child_line) * 8) // 2), 16, 1
        )
        timer_label = "STUDY " if presence["present"] else "LAST  "
        timer_line = timer_label + duration_text
        oled.text(
            timer_line, max(0, (128 - len(timer_line) * 8) // 2), 32, 1
        )
        ...
        oled.text(
            sensor_line, max(0, (128 - len(sensor_line) * 8) // 2), 48, 1
        )
        oled.show()
    ...
    time.sleep_ms(
        1 if normal_animator.active else (
