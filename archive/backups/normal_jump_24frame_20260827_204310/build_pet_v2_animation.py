...
ANIMATIONS = {
    "normal": {
        ...
        "accent": (255, 255, 255),
        # Eight output frames at roughly 8 FPS.  Four generated key poses are
        # expanded with deterministic vertical in-betweens.
        "frame_durations_ms": (130, 120, 120, 120, 180, 120, 120, 130),
...
def expand_jump_keyframes(
    ...
    """Create hard-edged in-betweens for a smoother eight-frame arc."""
    if len(keyframes) != 4:
        raise ValueError("the normal jump requires four generated keyframes")
    crouch, takeoff, apex, landing = keyframes
    return [
        crouch,
        shift_masks(crouch, -4),
        takeoff,
        shift_masks(takeoff, -4),
        apex,
        shift_masks(apex, 4),
        shift_masks(landing, -6),
        landing,
    ]
...
def main() -> None:
    ...
    print("built normal=8 frames, sick=4 frames, evolved=4 frames")
