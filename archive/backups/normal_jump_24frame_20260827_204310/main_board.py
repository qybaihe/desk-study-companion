...
from speaker_prompt import PCMVoicePlayer
from st7789 import ST7789
from study_reminder import OneShotStudyReminder
...
if lcd is not None:
    ...
    normal_jump_assets_ok = (
        ...
        and len(lcd_normal_animation_sprites) == 8
        and all(
...
# NORMAL stays on a still sprite that is byte-identical to the landing frame.
# After a full idle minute it performs one eight-frame jump, then waits another
# full minute.  Per-frame timing holds the apex slightly longer so the jump is
# easy to see without turning the pet into a continuously moving distraction.
NORMAL_ANIMATION_INTERVAL_MS = 60_000
NORMAL_ANIMATION_FRAME_DURATIONS_MS = (
    130, 120, 120, 120, 180, 120, 120, 130,
)
...
while True:
    ...
    if lcd is not None:
        ...
        jump_started = normal_animator.update(
            now,
            visual_state == "NORMAL" and normal_jump_assets_ok,
            time.ticks_diff,
        )
    ...
    voice_player.pump(time.ticks_ms())
    ...
    # Increase refresh precision only during the one-second normal jump.  Idle
    ...
    time.sleep_ms(
        30 if normal_animator.active or voice_player.playing else 200
    )
