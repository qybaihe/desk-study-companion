...
if __name__ == "__main__":
    ...
    upload(
        "assets/pets/lcd/pet_01_136x112_white.mono",
        "/pet_01_white.mono",
    )
    upload(
        "assets/pets/lcd/pet_01_136x112_accent.mono",
        "/pet_01_accent.mono",
    )
    for animation_name in ("sick", "evolved"):
        for frame_number in range(4):
            upload(
                "assets/pets/v2/lcd/%s_%d_white.mono"
                % (animation_name, frame_number),
                "/%s_%d_white.mono" % (animation_name, frame_number),
            )
            upload(
                "assets/pets/v2/lcd/%s_%d_accent.mono"
                % (animation_name, frame_number),
                "/%s_%d_accent.mono" % (animation_name, frame_number),
            )
