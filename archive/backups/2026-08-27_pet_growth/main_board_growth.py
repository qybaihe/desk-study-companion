    ...
    try:
        with open("/state.txt", "w") as state_file:
            ...
            state_file.write(
                ...
                "daily_study_seconds=%d daily_goal_percent=%d env_ok=%d\n"
                % (
...
WHITE = 0xFFFF
BLACK = 0x0000
GREEN = lcd_color(77, 215, 123)
RED = lcd_color(228, 30, 45)
PINK = lcd_color(255, 176, 190)
lcd_stage_bases = []
if lcd is not None:
    accent_colors = (WHITE, GREEN, RED, PINK)
    for stage_number in range(1, 5):
        white_mask = load_binary(
            "/pet_%02d_white.mono" % stage_number
        )
        accent_mask = load_binary(
            "/pet_%02d_accent.mono" % stage_number
        )
        draw_lcd_static(
            lcd_fb, white_mask, accent_mask,
            accent_colors[stage_number - 1], BLACK, WHITE,
        )
        lcd_stage_bases.append(bytes(lcd_buffer))
    lcd_buffer[:] = lcd_stage_bases[0]
    lcd.show(lcd_buffer)
...
while True:
    ...
    # LCD: four-stage pet cultivation with daily study and environment stats.
    if lcd is not None:
        lcd_buffer[:] = lcd_stage_bases[pet_state["stage"] - 1]
        light_percent = min(100, light_average * 100 // 4095)

        lcd_fb.text("PET LV%d" % pet_state["stage"], 8, 9, WHITE)
        if pet_state["sick"]:
            draw_sick_cross(lcd_fb, WHITE)
            scaled_text(lcd_fb, "SICK", 46, 156, 2, WHITE)
        else:
            scaled_text(lcd_fb, "MOMO", 46, 156, 2, WHITE)

        session_short = "%02d:%02d" % (
            min(99, study_seconds // 3600),
            (study_seconds % 3600) // 60,
        )
        lcd_fb.text("SESSION " + session_short, 17, 184, WHITE)
        lcd_fb.text(
            "GOAL 4H %3d%%" % pet_state["daily_goal_percent"],
            17, 202, WHITE,
        )
        ...
        growth_width = 56 * min(100, pet_state["growth"]) // 100
        if growth_width:
        ...
        lcd_fb.text(daily_text, 174, 110, WHITE)
        lcd_fb.text(
            "L%3d%% %s" % (
