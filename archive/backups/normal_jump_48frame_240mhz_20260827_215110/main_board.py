...
if lcd is not None:
    ...
    normal_jump_assets_ok = (
        ...
        and len(lcd_normal_animation_sprites) == 24
        and all(
...
# NORMAL plays a 24-frame, two-second jump every 60 seconds.  SICK and EVOLVED
...
NORMAL_ANIMATION_FRAME_DURATIONS_MS = (
    84, 83, 83, 84, 83, 83, 84, 83, 83, 84, 83, 83,
    84, 83, 83, 84, 83, 83, 84, 83, 83, 84, 83, 83,
)
...
while True:
    # Refill I2S before the full-frame LCD update and sensor work.  The larger
    # DMA buffer now carries roughly two seconds of 16 kHz mono PCM, so an
    # occasional slow ToF/DHT transaction cannot starve the speaker.
    ...
    voice_started = False
    if not voice_player.playing and voice_queue:
    ...
    if lcd is not None:
        ...
        if voice_player.playing:
            if voice_player.path == "/light_too_dark.pcm":
                bottom_message = "TURN ON LAMP"
            else:
                bottom_message = "TAKE A BREAK"
    ...
    time.sleep_ms(
        10 if normal_animator.active or voice_player.playing else 200
    )
