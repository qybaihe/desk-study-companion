...
240x240 ST7789 LCD: light readings plus DHT11 temperature/humidity.
...
def horizontal_bar(target, x, y, width, height, value, maximum, color):
    target.rect(x, y, width, height, color)
    fill_width = int((width - 4) * max(0, min(maximum, value)) / maximum)
    if fill_width:
        target.fill_rect(x + 2, y + 2, fill_width, height - 4, color)
...
while True:
    ...
    # LCD: dedicated two-channel light sensor screen.
    if lcd is not None:
        lcd_fb.fill(BLACK)
        lcd_fb.text("LIGHT SENSORS", 68, 12, WHITE)
        lcd_fb.hline(12, 28, 216, WHITE)

        scaled_text(lcd_fb, "L1 %4d" % v1, 20, 43, 2, WHITE)
        horizontal_bar(lcd_fb, 20, 70, 200, 18, v1, 4095, WHITE)
        lcd_fb.text("ADC GPIO6   %3d%%" % (v1 * 100 // 4095), 48, 94, WHITE)

        scaled_text(lcd_fb, "L2 %4d" % v2, 20, 120, 2, WHITE)
        horizontal_bar(lcd_fb, 20, 147, 200, 18, v2, 4095, WHITE)
        lcd_fb.text("ADC GPIO7   %3d%%" % (v2 * 100 // 4095), 48, 171, WHITE)

        lcd_fb.hline(12, 194, 216, WHITE)
        temperature_text = (
            "%2d" % temperature_c if temperature_c is not None else "--"
        )
        humidity_text = (
            "%2d" % humidity_percent if humidity_percent is not None else "--"
        )
        lcd_fb.text("TEMP: %s C" % temperature_text, 80, 207, WHITE)
        lcd_fb.text("HUM : %s %%" % humidity_text, 80, 224, WHITE)
        lcd.show(lcd_buffer)
    ...
    if (
        ...
        save_state(
            ...
            tof_healthy, lcd is not None, beijing_time,
        )
        last_state_save = now
