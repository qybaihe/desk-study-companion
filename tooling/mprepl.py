...
class Port:
    ...
    def exec(self, code: str, timeout: float = 10.0) -> tuple[str, str]:
        try:
            # A completed raw-REPL command leaves the interpreter at the next
            # raw prompt. Reusing that session avoids dozens of Ctrl-A/Ctrl-C
            # handshakes during asset verification and deployment.
            ...
            while data.count(4) < 2:
                if time.monotonic() >= deadline:
                    raise TimeoutError(
                        f"Execution timed out; received {bytes(data)!r}"
                    )
                chunk = self.ser.read(self.ser.in_waiting or 1)
                if chunk:
                    data.extend(chunk)
