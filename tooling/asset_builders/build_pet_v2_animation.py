...
ANIMATIONS = {
    "normal": {
        ...
        "sheet_grid": (3, 2),
        "accent": (255, 255, 255),
        # Exactly 2,000 ms across 48 frames: 24 FPS average.
        "frame_durations_ms": tuple(
            41 if index % 3 == 2 else 42 for index in range(48)
        ),
...
def expand_jump_keyframes(
    ...
    """Expand six generated key poses into a smooth 48-frame jump arc."""
    if len(keyframes) != 6:
    ...
    for index in range(frame_count):
        ...
        jump_height = round(17 * math.sin(math.pi * phase))
        bottom_positions.append(108 - jump_height)
    baseline = 108
    return [
...
def main() -> None:
    ...
    for state, spec in ANIMATIONS.items():
        ...
        if state == "normal":
            ...
            contact.resize((2176, 1344), Image.Resampling.NEAREST).save(
    ...
    print("built normal=48 frames/2 seconds, sick=4 frames, evolved=4 frames")
