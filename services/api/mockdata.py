"""演示数据。字段名与 iOS 的 Codable 模型逐一对应 ——
未接库时 App 拿到的结构和接库后完全一致，切换不需要改客户端代码。"""
...
MOCK = {
    "snapshot": {
        ...
        "todayComment": "今天开局不错——19 点后这一轮连续了 23 分钟，比平时同一时段长。"
                        "灯光偏暗过 8 分钟，小羊打了个哈欠。",
    },
    "eye": {
        "score": 92, "delta": 3,
    ...
    "study": {
        "todayMinutes": 52,
        "segments": [
            {"kind": "present", "minutes": 18},
            {"kind": "abnormal", "minutes": 8},
            {"kind": "present", "minutes": 11},
            {"kind": "away", "minutes": 21},
            {"kind": "present", "minutes": 23},
        ],
        "axis": ["18:10", "19:00", "19:42"],
        "longestMinutes": 23, "averageMinutes": 17, "awayCount": 2,
        "week": [
            {"label": "六", "minutes": 64, "isToday": False},
            {"label": "日", "minutes": 71, "isToday": False},
            {"label": "一", "minutes": 46, "isToday": False},
            {"label": "二", "minutes": 58, "isToday": False},
            {"label": "三", "minutes": 39, "isToday": False},
            {"label": "四", "minutes": 44, "isToday": False},
            {"label": "五", "minutes": 52, "isToday": True},
        ],
        "weekTotal": "共 5 小时 48 分",
        "hourly": [
            {"hour": 15, "value": 22}, {"hour": 16, "value": 38},
            {"hour": 17, "value": 45}, {"hour": 18, "value": 68},
            {"hour": 19, "value": 100}, {"hour": 20, "value": 74},
            {"hour": 21, "value": 30},
        ],
    ...
    "weekly": {
        ...
        "paragraphs": [
            "小满这周专注 5 小时 48 分，比上周多 36 分钟。多出来的时间几乎都在周二和周四晚上"
            "——这两天他连续坐满 40 分钟以上，中间没有离开桌子。",
            "护眼分从 87 升到 90。距离偏近的次数在减少，但仍然集中在每天 19 点以后，"
            "那是灯光最容易不够的时段。",
            "提醒响应率从 62% 升到 75%。台灯类的提醒他基本都会照做；"
            "「休息喝水」这一类有两次没有反应，下周可以留意。",
        ],
        "deltas": [
    ...
    "reminders": {
        ...
        "note": "「休息喝水」的响应率连续两周在降。同一句提示听多了会失效，"
                "可以换个说法或调冷却时间。",
    },
    "settings": {
