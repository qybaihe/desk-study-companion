...
durations = tuple(41 if index % 3 == 2 else 42 for index in range(48))
assert len(durations) == 48 and sum(durations) == 2_000
...
# The full 48-frame timeline finishes at exactly 2,000 ms.
now = 60_000
for duration in durations[:-1]:
    now += duration
    animator.update(now, True, straight_diff, straight_add)
    seen.append(animator.frame)
assert seen == list(range(48)), seen
now += durations[-1]
animator.update(now, True, straight_diff, straight_add)
assert now == 62_000
assert animator.frame == -1
assert animator.play_count == 1
assert animator.just_completed
assert animator.last_duration_ms == 2_000
...
for filename in ("normal.rgb565",) + tuple(
    "normal_%d.rgb565" % index for index in range(48)
):
